...
def generate_gemini_briefing(info, history, news, analysis_type):
    ...
    if analysis_type == '뉴스':
        ...
        - 50일 이동평균선: {ma50:.2f}
        - 200일 이동평균선: {ma200:.2f}
        - 52주 최고가/최저가: {info.get('fiftyTwoWeekHigh', 'N/A'):.2f} / {info.get('fiftyTwoWeekLow', 'N/A'):.2f}
    ...
    if info is None:
        ...
        with tab1:
            ...
            if st.session_state.gemini_briefing:
                st.markdown("---")
                if 'news' in st.session_state.gemini_briefing and st.session_state.gemini_briefing['news']:
                    with st.container(border=True):
                        st.markdown("##### 📰 뉴스 분석 요약")
                        st.write(st.session_state.gemini_briefing['news'])
                if 'chart' in st.session_state.gemini_briefing and st.session_state.gemini_briefing['chart']:
                    with st.container(border=True):
                        st.markdown("##### 📊 기술적 분석 요약")
                        st.write(st.session_state.gemini_briefing['chart'])
                if 'financials' in st.session_state.gemini_briefing and st.session_state.gemini_briefing['financials']:
                    with st.container(border=True):
                        st.markdown("##### 💰 재무 분석 요약")
                        st.write(st.session_state.gemini_briefing['financials'])
            else:
        ...
        with tab4:
            ...
            st.info("""**[안내]** 이 분석은 제미나이 AI가 실시간 웹을 검색하는 것이 아니라, 제공된 최신 데이터(뉴스, 주가, 재무)를 종합하여 애널리스트의 시각으로 **재해석한 분석**입니다.""")
