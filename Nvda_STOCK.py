...
def generate_ai_analysis(info, data, analysis_type):
    ...
    if analysis_type == 'verdict':
        ...
        **최종 투자 의견 및 전략:** (서론-본론-결론 형식으로, 모든 분석을 종합하여 최종 투자 등급('강력 매수', '매수 고려', '관망', '투자 주의' 중 하나)을 결정하고, 왜 그렇게 판단했는지에 대한 핵심적인 이유와 투자 전략을 논리적으로 설명해주세요.)"""
        ...
        with tab2:
            ...
            with st.container(border=True):
                 ...
                 if financials is not None and not financials.empty:
                    st.dataframe(financials.T.iloc[:4]) # 최근 4분기 재무제표 표시
                 else: st.info("재무 데이터를 가져올 수 없습니다.")

        with tab3:
            with st.container(border=True):
                st.subheader("📰 최신 뉴스 원문 (From Google News)")
                if news:
